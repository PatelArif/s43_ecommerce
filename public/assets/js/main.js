(function($) {
    ...
    $documentOn.ready( function() {
        ...
        if ($(".instagram-banner-slider-2").length > 0) {
            const instagrambannerSlider2 = new Swiper(
                ".instagram-banner-slider-2",
                {
                    spaceBetween: 30,
                    speed: 2000,
                    loop: true,
        ...
        priceInput.forEach((input) => {
            input.addEventListener("input", (e) => {
                ...
                if (
                    ...
                    maxPrice <= parseInt(rangeInput[1].max, 10)
                ) {
                    if (e.target.className === "input-min") {
                        rangeInput[0].value = minPrice;
                        range.style.left =
                            (minPrice / parseInt(rangeInput[0].max, 10)) * 100 +
                            "%";
                    } else {
                        rangeInput[1].value = maxPrice;
                        range.style.right =
                            100 -
                            (maxPrice / parseInt(rangeInput[1].max, 10)) * 100 +
                            "%";
                    }
                }
            });
        ...
        rangeInput.forEach((input) => {
            input.addEventListener("input", (e) => {
                ...
                if (maxVal - minVal < priceGap) {
                    ...
                    range.style.left =
                        (minVal / parseInt(rangeInput[0].max, 10)) * 100 + "%";
                    range.style.right =
                        100 -
                        (maxVal / parseInt(rangeInput[1].max, 10)) * 100 +
                        "%";
                }
            });
        });
        ...
        const quantityIncrement =
            document.querySelectorAll(".quantityIncrement");
        const quantityDecrement =
            document.querySelectorAll(".quantityDecrement");

        // INCREMENT
        quantityIncrement.forEach((btn) => {
            btn.addEventListener("click", function () {
                ...
                $(input).val(value); // Ensure jQuery reads correct value
            });
        });

        // DECREMENT
        quantityDecrement.forEach((btn) => {
            btn.addEventListener("click", function () {
                let input = this.parentElement.querySelector(
                    ".cart-quantity-input"
                );
        ...
        // ADD TO CART BUTTON (Only sends when clicked)
        $(document).on("click", ".cart-action-btn", function () {
            let pid = $(this).data("id");
            let url = $(this).data("url");

            // Fetch updated quantity correctly
            let qty = $(
                '.cart-quantity-input[data-product-id="' + pid + '"]'
            ).val();
