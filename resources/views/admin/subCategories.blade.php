    ...
    <div id="layoutSidenav_content">
                        ...
                        <tbody>
                            @foreach ($subCategories as $index => $subcategory)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <h5>{{ $subcategory->category->name ?? '—' }}</h5>
                                    </td> {{-- Show related category --}}
                                    <td class="text-center">
                                        @if ($subcategory->image)
                                            <img src="{{ asset(config('constants.IMAGE_PATH') . $subcategory->image) }}"
                                                alt="Subcategory Image" width="100">
                                        @else
        ...
        <script>
            ...
            function openEditModal(id, name, image, categoryId) {
                ...
                let image_path = "{{ config('constants.IMAGE_PATH') }}";
          
                let imageHtml = image ?
                    `<img src="/${image_path}${image}" width="70" alt="Current Image">` :
                    '<span class="text-muted">No Image</span>';
